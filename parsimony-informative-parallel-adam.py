...
def find_ps(alignment,width,seq,remove_fq,epis_base):
    # Default value
    result = [-1,-1]
    uniq_nc = {}
    tmp_all = 0
...
def find_ps_sites(al, seq, remove_fq, epis_base):
    rm_pis = []
    all_pis = []
    alignment = al
    result_list = []
    p = Pool(cpu_count())
    for width in tqdm(range(0, len(alignment[0])), desc='First step: find ePIS'):
        result_list.append(p.apply_async(find_ps,args=(alignment,width,seq,remove_fq,epis_base,)))
    p.close()
    p.join()
